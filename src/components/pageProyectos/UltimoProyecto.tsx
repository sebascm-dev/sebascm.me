        ...
        <>
            {projects?.map((project) => {
                ...
                return (
                    <article
                        key={project.id}
                        className='relative mt-10 border border-[#2E2D2D] rounded-md p-4 bg-[#1C1C1C]/50 shadow-lg backdrop-blur-[2px] hover:border-[#EDEDED]/30 transition-colors duration-300 mb-6 flex flex-col md:flex-row gap-2'
                    >
                        <div className='mt-5 md:mt-0 md:w-[40%]'>
                            <img
                                src={project.imagen}
                                alt={project.titulo}
                                className='aspect-video object-cover h-60 rounded-sm'
                            />
                        </div>
