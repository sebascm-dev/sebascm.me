...
type DayStatus = {
    date: string;
    played: boolean;
    resultado?: number; // 1 = Partido Ganado, 0 = Partido Perdido, 2 = Entrenamiento
};
            ...
            <article className="">
                    ...
                    <div className="flex flex-wrap">
                        {days.map((day, index) => (
                            <motion.div
                                ...
                                className={`w-3 h-3 m-[1px] rounded-sm cursor-pointer scale-100 hover:scale-125 transition-transform duration-300 ${day.played
                                        ? day.resultado === 0
                                            ? "bg-red-500"
                                            : day.resultado === 2
                                                ? "bg-yellow-500"
                ...
                <div className="flex items-center justify-end text-xs text-gray-100/50 absolute bottom-4 right-4">
                    <div className="flex items-center gap-1">
                        ...
                        <span className="ml-1">Ganados: {totalGanados}</span>
                    </div>
                    <div className="flex items-center gap-1 ml-4">
                        <div className="w-3 h-3 m-[1px] rounded-sm bg-red-500"></div>
                        <span className="ml-1">Perdidos: {totalPerdidos}</span>
                    </div>
                    <div className="flex items-center gap-1 ml-4">
                        <div className="w-3 h-3 m-[1px] rounded-sm bg-yellow-500"></div>
                        <span className="ml-1">Entrenamiento: {totalEntrenamiento}</span>
                    </div>
                </div>
            </article>
        </div>
